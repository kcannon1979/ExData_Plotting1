...
file1$Voltage <- as.numeric(file1$Voltage, digits = 6)

#Create graphs in order and write to png file.
png("plot4.png", width = 480, height = 480)
par(mfrow = c(2,2))
plot2 <- plot(file1$Datetime, file1$Global_active_power, pch = ".", xlab = "", 
              ylab = "Global Active Power (kilowatts)")
lines(file1$Datetime, file1$Global_active_power)
plot3 <- plot(file1$Datetime, file1$Voltage, xlab = "datetime", ylab = "Voltage",
              type = 'l')
plot4 <- plot(file1$Datetime, file1$Sub_metering_1, pch = ".", xlab = "", 
...
plot5 <- plot(file1$Datetime, file1$Global_reactive_power, type = "l",
